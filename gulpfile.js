...
var EVN_DEV = 'development';
var EVN_PROD = 'production';
var EVN_DTEST = 'dtest';
var EVN_KTEST = 'ktest';
var EVN_CTEST = 'ctest';
// Local settings
...
gulp.task('env:dev', function () {
  process.env.NODE_ENV = EVN_DEV;
});

// Set NODE_ENV to 'production'
gulp.task('env:prod', function () {
  process.env.NODE_ENV = EVN_PROD;
});

// Set NODE_ENV to 'ktest'
gulp.task('env:ktest', function () {
  process.env.NODE_ENV = EVN_KTEST;
});

// Set NODE_ENV to 'dtest'
gulp.task('env:dtest', function () {
  process.env.NODE_ENV = EVN_DTEST;
});

// Set NODE_ENV to 'ktest'
gulp.task('env:ctest', function () {
  process.env.NODE_ENV = EVN_CTEST;
});
...
gulp.task('prod', function (done) {
  runSequence(['copyLocalEnvConfig', 'makeUploadsDir', 'templatecache'], 'build', 'env:prod', 'lint', ['nodemon-nodebug'], done);
});

// Run the project in ctest mode
gulp.task('ctest', function (done) {
  runSequence(['copyLocalEnvConfig', 'makeUploadsDir', 'templatecache'], 'build', 'env:ctest', 'lint', ['nodemon-nodebug'], done);
});

// Run the project in dtest mode
gulp.task('dtest', function (done) {
  runSequence(['copyLocalEnvConfig', 'makeUploadsDir', 'templatecache'], 'build', 'env:dtest', 'lint', ['nodemon-nodebug'], done);
});

// Run the project in ktest mode
gulp.task('ktest', function (done) {
  runSequence(['copyLocalEnvConfig', 'makeUploadsDir', 'templatecache'], 'build', 'env:ktest', 'lint', ['nodemon-nodebug'], done);
});
...
// Run Mongo Seed with ctest environment config
gulp.task('seed:ctest', function (done) {
  runSequence('env:ctest', 'mongo-seed', done);
});

// Run Mongo Seed with ktest environment config
gulp.task('seed:ktest', function (done) {
  runSequence('env:ktest', 'mongo-seed', done);
});

// Run Mongo Seed with dtest environment config
gulp.task('seed:dtest', function (done) {
  runSequence('env:dtest', 'mongo-seed', done);
});
