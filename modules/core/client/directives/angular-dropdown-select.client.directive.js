...
var directiveModule = angular.module('core');

directiveModule.directive('mfDropdownStaticInclude', mfDropdownStaticInclude);

mfDropdownStaticInclude.$inject = ['$compile'];
function mfDropdownStaticInclude($compile) {
...
directiveModule.directive('ngDropdownMultiselect', ngDropdownMultiselect);
...
function ngDropdownMultiselect($filter, $document, $compile, $parse) {
  return {
    ...
    template: function (element, attrs) {
      ...
      var template = '<div class="multiselect-parent btn-group dropdown-multiselect" ng-class="{open: open}"  style="width: 100%">';
      template += '<button ng-disabled="disabled" style="width: 100%" type="button" class="dropdown-toggle" ng-class="settings.buttonClasses" ng-click="toggleDropdown()">{{getButtonText()}}&nbsp;<span class="caret"></span></button>';
    ...
    link: function ($scope, $element, $attrs) {
      ...
      $scope.getButtonText = function () {
        if ($scope.settings.dynamicTitle && ($scope.selectedModel && $scope.selectedModel.length > 0 || ($scope.selectedModel && angular.isObject($scope.selectedModel) && Object.keys($scope.selectedModel).length > 0))) {
